...
const Data := preload('character_policeman_state.gd')

var state: Data = load("res://game/characters/policeman/character_policeman.tres")
...
func _on_click() -> void:
	...
	await C.Policeman.say("Hola soy un Policia")
	await C.Policeman.say("aquì el Policia dice cosas")
	R.goto_room('Policeman')
