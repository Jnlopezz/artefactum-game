...
const Data := preload('character_manager_state.gd')

var state: Data = load("res://game/characters/manager/character_manager.tres")
...
func _on_click() -> void:
	...
	await C.Manager.say("Hola soy un Encargado")
	await C.Manager.say("aquì el Encargado dice cosas")
	R.goto_room('Manager')
